-- BiS data: Guardian Druid — Midnight Season 1
-- Source: https://www.wowhead.com/guide/classes/druid/guardian/bis-gear
--         https://www.wowhead.com/guide/classes/druid/guardian/stat-priority-pve-tank
-- Last updated: 2026-03-14

BiSHelper_Data = BiSHelper_Data or {}

BiSHelper_Data["DRUID_GUARDIAN"] = {
    label = "Guardian Druid",
    class = "DRUID",
    spec  = "Guardian",

    -- Ursoc's Fury & Wildstalker: identical priority
    statPriority = {
        raid = {
            note  = "Defensively Haste+Vers valued; all secondaries close in value",
            stats = {
                { name = "Haste",           op = ">",   r = 1.00, g = 0.82, b = 0.20 },
                { name = "Versatility",     op = ">",   r = 0.65, g = 0.40, b = 1.00 },
                { name = "Critical Strike", op = ">",   r = 1.00, g = 0.35, b = 0.35 },
                { name = "Mastery",         op = nil,   r = 0.30, g = 0.75, b = 1.00 },
            },
        },
        mythicplus = {
            note  = "Same priority as Raid — trinkets are very flexible",
            stats = {
                { name = "Haste",           op = ">",   r = 1.00, g = 0.82, b = 0.20 },
                { name = "Versatility",     op = ">",   r = 0.65, g = 0.40, b = 1.00 },
                { name = "Critical Strike", op = ">",   r = 1.00, g = 0.35, b = 0.35 },
                { name = "Mastery",         op = nil,   r = 0.30, g = 0.75, b = 1.00 },
            },
        },
        dr = {
            { name = "Haste",          rating = 1320, r = 1.00, g = 0.82, b = 0.20 },
            { name = "Crit / Mastery", rating = 1380, r = 1.00, g = 0.60, b = 0.30 },
            { name = "Versatility",    rating = 1620, r = 0.65, g = 0.40, b = 1.00 },
        },
    },

    content = {
        -- ── Raid BiS ────────────────────────────────────────
        -- Guardian uses a 2H weapon (staff/mace)
        raid = {
            [1]  = { itemID = 249913, name = "Mask of Darkest Intent",            source = "Seat of the Triumvirate"         }, -- Head
            [2]  = { itemID = 251096, name = "Pendant of Aching Grief",            source = "Windrunner Spire"                }, -- Neck
            [3]  = { itemID = 250022, name = "Seedpods of the Luminous Bloom",     source = "Tier Set"                        }, -- Shoulder
            [5]  = { itemID = 250027, name = "Trunk of the Luminous Bloom",        source = "Tier Set"                        }, -- Chest
            [6]  = { itemID = 249374, name = "Scorn-Scarred Shul'ka's Belt",       source = "Chimaerus · The Dreamrift"       }, -- Waist
            [7]  = { itemID = 250023, name = "Phloemwraps of the Luminous Bloom",  source = "Tier Set"                        }, -- Legs
            [8]  = { itemID = 249334, name = "Void-Claimed Shinkickers",           source = "Imperator Averzian"              }, -- Feet
            [9]  = { itemID = 249327, name = "Void-Skinned Bracers",               source = "Vorasius"                        }, -- Wrist
            [10] = { itemID = 250025, name = "Arbortenders of the Luminous Bloom", source = "Tier Set"                        }, -- Hands
            [11] = { itemID = 251093, name = "Omission of Light",                  source = "Nexus Point Xenas"               }, -- Ring 1
            [12] = { itemID = 251217, name = "Occlusion of Void",                  source = "Nexus Point Xenas"               }, -- Ring 2
            [13] = { itemID = 249343, name = "Gaze of the Alnseer",                source = "Chimaerus · The Dreamrift"       }, -- Trinket 1
            [14] = { itemID = 193701, name = "Algeth'ar Puzzle Box",               source = "Algeth'ar Academy"               }, -- Trinket 2
            [15] = { itemID = 249370, name = "Draconic Nullcape",                  source = "Vaelgor & Ezzorak · Voidspire"   }, -- Back
            [16] = { itemID = 249278, name = "Alnscorned Spire",                   source = "Chimaerus · The Dreamrift"       }, -- Main Hand (2H)
        },

        -- ── Mythic+ BiS ──────────────────────────────────────
        -- Difference: Trinket 2 (Heart of Wind preferred in M+)
        mythicplus = {
            [1]  = { itemID = 249913, name = "Mask of Darkest Intent",            source = "Seat of the Triumvirate"         }, -- Head
            [2]  = { itemID = 251096, name = "Pendant of Aching Grief",            source = "Windrunner Spire"                }, -- Neck
            [3]  = { itemID = 250022, name = "Seedpods of the Luminous Bloom",     source = "Tier Set"                        }, -- Shoulder
            [5]  = { itemID = 250027, name = "Trunk of the Luminous Bloom",        source = "Tier Set"                        }, -- Chest
            [6]  = { itemID = 249374, name = "Scorn-Scarred Shul'ka's Belt",       source = "Chimaerus · The Dreamrift"       }, -- Waist
            [7]  = { itemID = 250023, name = "Phloemwraps of the Luminous Bloom",  source = "Tier Set"                        }, -- Legs
            [8]  = { itemID = 249334, name = "Void-Claimed Shinkickers",           source = "Imperator Averzian"              }, -- Feet
            [9]  = { itemID = 249327, name = "Void-Skinned Bracers",               source = "Vorasius"                        }, -- Wrist
            [10] = { itemID = 250025, name = "Arbortenders of the Luminous Bloom", source = "Tier Set"                        }, -- Hands
            [11] = { itemID = 251093, name = "Omission of Light",                  source = "Nexus Point Xenas"               }, -- Ring 1
            [12] = { itemID = 251217, name = "Occlusion of Void",                  source = "Nexus Point Xenas"               }, -- Ring 2
            [13] = { itemID = 193701, name = "Algeth'ar Puzzle Box",               source = "Mythic+"                         }, -- Trinket 1
            [14] = { itemID = 250256, name = "Heart of Wind",                      source = "Mythic+"                         }, -- Trinket 2 (M+)
            [15] = { itemID = 249370, name = "Draconic Nullcape",                  source = "Vaelgor & Ezzorak · Voidspire"   }, -- Back
            [16] = { itemID = 249278, name = "Alnscorned Spire",                   source = "Chimaerus · The Dreamrift"       }, -- Main Hand (2H)
        },
    },
}
